...
private: 
    ...
    template <typename Policy, typename DocumentPredicate> 
    std::vector<Document> FindAllDocuments(const Policy& policy, const SearchServer::Query& query, 
                                      DocumentPredicate document_predicate) const;
...
std::vector<Document> SearchServer::FindTopDocuments(const Policy& policy, const std::string_view& raw_query, DocumentStatus status) const {
    ...
    template <typename Policy, typename DocumentPredicate>
    std::vector<Document> SearchServer::FindAllDocuments(const Policy& policy, const SearchServer::Query& query,
                                      DocumentPredicate document_predicate) const {
